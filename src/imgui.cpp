...
void MyImGui::Setup(const vk::RenderPass& renderPass, Viewport& viewport)
{
    ...
    float iconFontSize = baseFontSize * 2.0f; // FontAwesome fonts need to have their sizes reduced by 2.0f/3.0f in order to align correctly
    ...
    for (int i = 0; i < viewportDescriptorSets_.size(); i++)
        viewportDescriptorSets_[i] = ImGui_ImplVulkan_AddTexture(viewport.frames[i].viewportImage.GetHandle().sampler,
                                                                 viewport.frames[i].viewportImage.GetHandle().imageView,
...
void MyImGui::Draw(std::unique_ptr<Scene>& scene, Viewport& viewport, size_t frameIndex)
{
    ...
    if (ImGui::IsKeyPressed(ImGuiKey_Delete) && itemSelected_ > -1) {
        scene->DeleteMesh(itemSelected_, scene->meshes[itemSelected_].GetMeshType());
        itemSelected_ = -1;
    }
...
void MyImGui::SetCameraControl(std::unique_ptr<Scene>& scene)
{
    if (ImGui::IsKeyPressed(ImGuiKey_C)) {
        scene->camera.SetCameraControl();
        ImGuiIO& io = ImGui::GetIO();

        if (scene->camera.IsControllable()) {
...
{
    viewport.frames[frameIndex].command.TransitImageLayout(&viewport.frames[frameIndex].viewportImage,
    ...
    viewport.frames[frameIndex].command.Submit();
    ...
    if (viewport.panelRatio != viewportPanelRatio || viewport.outDated) {
        viewport.panelRatio = viewportPanelRatio;
...
void MyImGui::DrawObjectWindow(std::unique_ptr<Scene>& scene)
{
        ...
        for (int i = 0; i < scene->meshes.size(); i++) {
            ...
            if (ImGui::BeginPopupContextItem()) {
                if (ImGui::MenuItem("delete")) {
                    scene->DeleteMesh(i, scene->meshes[i].GetMeshType());
                    itemSelected_ = -1;
                }
                ImGui::EndPopup();
            }
    ...
    if (itemSelected_ > -1) {
        ...
        ImGui::Text("%s", scene->GetMeshName(itemSelected_));
